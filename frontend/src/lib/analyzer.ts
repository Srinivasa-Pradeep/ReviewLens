...
export interface AspectSet {
  summary: string;
  pros: { term: string; count: number; percentage: number }[];
  cons: { term: string; count: number; percentage: number }[];
  thematic: { category: string; score: number }[];
  comparison: string;
  highlights: { category: string; quote: string; sentiment: string }[];
  complaints: { issue: string; impact_score: number; volume: number; severity: string; root_cause: string }[];
  timeline: { stage_or_time: string; issue: string; diagnostic: string }[];
  requests: { feature: string; count: number; sample_quote: string }[];
  clusters: { category: string; issue: string; frequency: number }[];
  fixes: string[];
  improvements: string[];
  roadmap: string[];
  gaps: string[];
...
export function generateDynamicTemplate(productName: string): AspectSet {
  const nameLower = productName.toLowerCase();

  // 1. Check for cosmetics / beauty products
  const isCosmetics = /\b(lipstick|lip|gloss|balm|makeup|cosmetics|eyeliner|mascara|cream|lotion|foundation|concealer|shampoo|skincare|serum|polish|perfume|shadow|blush)\b/i.test(nameLower);
  
  // 2. Check for drinkware / bottles
  const isDrinkware = /\b(bottle|flask|thermos|mug|cup|tumbler|glass|canteen|jug|pitcher|shaker|beaker)\b/i.test(nameLower);

  // 3. Check for bags / luggage
  const isBags = /\b(bag|backpack|case|sleeve|wallet|pouch|purse|briefcase|luggage|suitcase|holder|pack|organizer|organiser)\b/i.test(nameLower);

  // Determine subcategory or category matching detectCategoryFromText
  let cat = "";
  if (isCosmetics) cat = "cosmetics";
  else if (isDrinkware) cat = "drinkware";
  else if (isBags) cat = "bags";
  else {
    // Falls back to standard categories
    if (/\b(watch|band|fitbit|wearable|tracker|smartwatch|fitness)\b/i.test(nameLower)) cat = "wearable";
    else if (/\b(coffee|brewer|kettle|maker|mug|cooker|blender|pot|grinder|cook|toaster|oven|airfryer|juicer)\b/i.test(nameLower)) cat = "kitchen";
    else if (/\b(shoe|boot|sneaker|running|shirt|pants|jacket|coat|hoodie|sock|apparel|clothing|jeans|tshirt|t-shirt|dress)\b/i.test(nameLower)) cat = "apparel";
    else if (/\b(phone|smartphone|iphone|android|galaxy|pixel|oneplus|cellular|mobile)\b/i.test(nameLower)) cat = "phone";
    else if (/\b(mouse|mice|trackpad|scroll|pointer|dpi|sensor|grip|clicks)\b/i.test(nameLower)) cat = "mouse";
    else if (/\b(keyboard|keypad|typing|keeb|keys|clicky|keycaps)\b/i.test(nameLower)) cat = "keyboard";
    else if (/\b(mic|microphone|headphone|headset|earphone|earbud|speaker|audio|sound|voice|music|earphones|earbuds|headphones|headsets|microphones|speakers|jbl|soundbar|soundbars)\b/i.test(nameLower)) cat = "audio";
    else if (/\b(monitor|screen|display|tv|television|displayport|hdmi|projector)\b/i.test(nameLower)) cat = "display";
    else if (/\b(book|read|manifest|novel|bestseller|paperback|hardcover|author|pages|reading|novels|books|hardback|paperbacks|literature|fiction|biography)\b/i.test(nameLower)) cat = "book";
    else if (/\b(organizer|organiser|desk|chair|table|shelf|furniture|storage|rack|stand|holder|riser|lamp|light|decor|cabinet|drawer|sofa|bed|stool)\b/i.test(nameLower)) cat = "home_office";
  }

  // Setup dynamic templates
  if (cat === "cosmetics") {
    return {
      summary: `Reviewers of ${productName} are generally pleased with its rich pigment intensity and smooth wear. It applies evenly and feels lightweight. However, some note a drying effect after a few hours and mention the cap could close more securely.`,
      pros: [
        { term: "Rich Color Payoff", count: 24, percentage: 84.0 },
        { term: "Smooth Gliding Wear", count: 20, percentage: 76.0 },
        { term: "Lightweight Formula", count: 15, percentage: 65.0 },
        { term: "Vibrant Shade Range", count: 11, percentage: 50.0 }
      ],
      cons: [
        { term: "Dry Matte Texture", count: 18, percentage: 52.0 },
        { term: "Transfers under Contact", count: 12, percentage: 38.0 },
        { term: "Fragile Snap Cap", count: 8, percentage: 25.0 }
      ],
      thematic: [
        { category: "Pigment Quality", score: 4.6 },
        { category: "Application Smoothness", score: 4.5 },
        { category: "Hydration Comfort", score: 3.4 },
        { category: "Packaging Build", score: 3.7 }
      ],
      comparison: `Compared to standard makeup, ${productName} offers exceptionally deep pigments for the price, though it lacks the all-day moisturizing properties of specialty lip balms.`,
      highlights: [
        { category: "Color", quote: "The shade is absolutely gorgeous and stays vibrant for hours.", sentiment: "Positive" },
        { category: "Feel", quote: "A bit dry after a few hours of wear, definitely need base hydration.", sentiment: "Negative" },
        { category: "Scent", quote: "Very pleasant subtle fragrance, not overwhelming at all.", sentiment: "Positive" }
      ],
      complaints: [
        { issue: "Drying or Tightness feeling on skin", impact_score: 8, volume: 18, severity: "High", root_cause: "High ratio of oil-absorbing matte styling powders in base formula." },
        { issue: "Cap cracking inside storage bags", impact_score: 6, volume: 12, severity: "Medium", root_cause: "Thin-walled plastic molding susceptible to compression stress." },
        { issue: "Pigment transfer on cups and clothing", impact_score: 5, volume: 10, severity: "Low", root_cause: "Absence of strong film-forming lock-in agents." }
      ],
      timeline: [
        { stage_or_time: "Upon Application", issue: "High-impact pigment color", diagnostic: "Formulation coats skin/lips smoothly with bold and rich coverage." },
        { stage_or_time: "Hour 3", issue: "Moisture evaporation begins", diagnostic: "Lipid barrier dries as volatile solvents evaporate from surface layer." },
        { stage_or_time: "Month 2", issue: "Snap closure friction wear", diagnostic: "Tiny plastic alignment ridge wears flat, loosening the cap lock." }
      ],
      requests: [
        { feature: "Add Jojoba or Shea butter oils", count: 22, sample_quote: "Please make the formula more moisturizing for long wear times." },
        { feature: "Thicker Magnetic Cap Closure", count: 14, sample_quote: "Improve the casing cap so it won't pop open and ruin my makeup bag." }
      ],
      clusters: [
        { category: "Product Quality", issue: "Shades differ from digital catalog", frequency: 9 },
        { category: "User Experience", issue: "Drying and flaking issues", frequency: 18 },
        { category: "Delivery", issue: "Arrived with broken tip", frequency: 5 },
        { category: "Customer Service", issue: "Replaced broken item quickly", frequency: 12 },
        { category: "Pricing", issue: "Affordable premium cosmetic choice", frequency: 15 }
      ],
      fixes: [
        "Incorporate natural plant seed oils to retain lip moisture barrier.",
        "Redesign snap cap connection thickness to increase stress durability."
      ],
      improvements: [
        "Include a micro-lip liner tool in standard packages.",
        "Add a secure magnetic locking mechanism to premium product lines."
      ],
      roadmap: [
        "Formulate an organic clean-beauty version targeting sensitive skin.",
        "Develop a transfer-resistant setting lock spray companion product."
      ],
      gaps: [
        "Our color density outclasses competitors, but we lag in long-term wear hydration.",
        "Competitors package individual travel sleeves which our catalog currently lacks."
      ],
      reviews: [
        `The pigment density of this ${productName} is absolutely beautiful. Highly recommend!`,
        `Nice color payoff, but it starts drying out my lips after about 3 hours.`,
        `Decent quality for the money, but the plastic cap feels cheap and could crack easily.`,
        `Smooth gliding application and very lightweight. One of my new favorite beauty items!`,
        `Love the bold matte look, but be careful as it transfers on cups and masks.`
      ]
    };
  } else if (cat === "drinkware") {
    return {
      summary: `Reviewers of ${productName} highlight its outstanding thermal insulation and rugged build. It keeps liquids cold or hot for hours and maintains a leak-proof seal. However, the complex lid parts can be tedious to clean.`,
      pros: [
        { term: "Long Thermal Retention", count: 26, percentage: 86.0 },
        { term: "Leak-Proof Lid Gasket", count: 21, percentage: 78.0 },
        { term: "Sturdy Carrying Handle", count: 14, percentage: 65.0 },
        { term: "Premium Powder Coat Finish", count: 10, percentage: 48.0 }
      ],
      cons: [
        { term: "Tedious to Clean Lid", count: 16, percentage: 50.0 },
        { term: "Scratches / Dents on Bottom", count: 12, percentage: 38.0 },
        { term: "Heavy weight when Filled", count: 9, percentage: 25.0 }
      ],
      thematic: [
        { category: "Thermal Retention", score: 4.8 },
        { category: "Durability", score: 4.4 },
        { category: "Cleanability", score: 3.2 },
        { category: "Portability", score: 4.1 }
      ],
      comparison: `The ${productName} retains temperatures longer than plastic single-wall bottles, but is noticeably heavier to carry during long hikes.`,
      highlights: [
        { category: "Insulation", quote: "Water stays freezing cold even after leaving the bottle in my car on a hot day.", sentiment: "Positive" },
        { category: "Lid", quote: "Getting tea residue out of the straw hinge crevices is nearly impossible without a special tool.", sentiment: "Negative" },
        { category: "Finish", quote: "The powder coating looks nice but chipped on the bottom edges after my first drop.", sentiment: "Neutral" }
      ],
      complaints: [
        { issue: "Hard to wash straw crevices", impact_score: 8, volume: 16, severity: "High", root_cause: "Narrow internal angles trap organic material." },
        { issue: "Chipping paint / powder finishes", impact_score: 6, volume: 12, severity: "Medium", root_cause: "Insufficient curing heat during production line coating." },
        { issue: "Leaking around silicone gaskets", impact_score: 7, volume: 10, severity: "High", root_cause: "Gasket ring slips out of alignment when lid is screwed tight." }
      ],
      timeline: [
        { stage_or_time: "Day 1 (First Fill)", issue: "Excellent temperature hold", diagnostic: "Vacuum insulation prevents condensation sweat entirely." },
        { stage_or_time: "Month 1", issue: "Residual beverage odor in seal", diagnostic: "Silicone compound absorbs molecules from coffee and tea." },
        { stage_or_time: "Month 3", issue: "Bottom scuffs and dents", diagnostic: "Steel base shell dents from drop impacts on concrete." }
      ],
      requests: [
        { feature: "Include custom straw brush tool", count: 24, sample_quote: "Please bundle a thin cleaning brush to scrub inside the drink nozzle." },
        { feature: "Protective silicone base boot", count: 15, sample_quote: "Add a rubber boot at the bottom to absorb impacts and prevent scratches." }
      ],
      clusters: [
        { category: "Product Quality", issue: "Insulation failure over time", frequency: 8 },
        { category: "User Experience", issue: "Difficult lid cleaning", frequency: 16 },
        { category: "Delivery", issue: "Received with dented bottom", frequency: 5 },
        { category: "Customer Service", issue: "Prompt replacement of leaking seal", frequency: 10 },
        { category: "Pricing", issue: "Great value insulated drinkware", frequency: 18 }
      ],
      fixes: [
        "Incorporate a wider mouth and simplified lid layout to ease hand-washing.",
        "Upgrade powder curing process to increase paint bonding durability."
      ],
      improvements: [
        "Provide a removable straw mouthpiece for straightforward washing.",
        "Add a non-slip silicone bumper sleeve at the bottom."
      ],
      roadmap: [
        "Develop a double-walled titanium model to reduce weight by 35%.",
        "Integrate a smart temperature indicator LCD panel into the top cap."
      ],
      gaps: [
        "Our thermal retention matches leading flasks, but we lack quick-click straw openings.",
        "Competitors offer laser-engraved personalizing details which we currently lack."
      ],
      reviews: [
        `Keeps my water ice-cold for 24 hours. The ${productName} is a lifesaver on warm days!`,
        `Amazing insulation, but scrubbing the inner straw hinge is very annoying.`,
        `Sturdy build, but the powder coat paint chipped near the base after a short drop.`,
        `Fits perfectly in my car cup holder and never leaks in my bag.`,
        `A bit heavy when filled with water, but the thermal performance is worth the weight.`
      ]
    };
  } else if (cat === "bags") {
    return {
      summary: `Reviewers of ${productName} emphasize its spacious storage capacity and durable, water-resistant fabrics. The strap layout is comfortable. However, the interior padding is thin, and stress seams show early signs of fraying.`,
      pros: [
        { term: "Excellent Storage Volume", count: 28, percentage: 88.0 },
        { term: "Water-Resistant Coating", count: 22, percentage: 78.0 },
        { term: "Ergonomic Cushioning", count: 16, percentage: 65.0 },
        { term: "Smooth Gliding Zippers", count: 11, percentage: 48.0 }
      ],
      cons: [
        { term: "Thin Protection Padding", count: 15, percentage: 50.0 },
        { term: "Fraying Seam Stitching", count: 10, percentage: 32.0 },
        { term: "Plastic Hardware Clips", count: 7, percentage: 22.0 }
      ],
      thematic: [
        { category: "Storage Capacity", score: 4.8 },
        { category: "Material Quality", score: 4.2 },
        { category: "Seam Strength", score: 3.2 },
        { category: "Comfort", score: 4.4 }
      ],
      comparison: `The ${productName} provides superior internal pocket layout compared to simple sleeves, but has less drop protection than heavy-duty hard cases.`,
      highlights: [
        { category: "Pockets", quote: "The internal divider sleeves fit my laptop and tech gear perfectly.", sentiment: "Positive" },
        { category: "Padding", quote: "Bottom padding is virtually non-existent. Need to set the bag down very gently.", sentiment: "Negative" },
        { category: "Hardware", quote: "Strap adjustment buckles are thin plastic, feel like they could snap.", sentiment: "Neutral" }
      ],
      complaints: [
        { issue: "Stitching separation at handle seams", impact_score: 8, volume: 15, severity: "High", root_cause: "Single thread stitch construct used on high tension joints." },
        { issue: "Weak padding at the bottom corners", impact_score: 7, volume: 10, severity: "Medium", root_cause: "Thin 2mm sponge inserts failing under vertical impact." },
        { issue: "Zipper slider teeth alignment jam", impact_score: 6, volume: 7, severity: "Medium", root_cause: "Suboptimal zipper track tension gauge." }
      ],
      timeline: [
        { stage_or_time: "Day 1 (Unboxing)", issue: "Excellent pocket layout", diagnostic: "Multiple compartments provide efficient storage for everyday use." },
        { stage_or_time: "Week 3", issue: "Fraying seam threads", diagnostic: "Fabric friction at straps loosens single-lock stitches." },
        { stage_or_time: "Month 3", issue: "Zipper track starts sticking", diagnostic: "Dirt gathers in track or sliders widen slightly from pulling." }
      ],
      requests: [
        { feature: "Double-reinforced box stitching", count: 22, sample_quote: "Please strengthen the top handle seams so they don't split when heavy." },
        { feature: "High-density foam corner cushions", count: 16, sample_quote: "Add thick rubber corner pads to protect laptops from direct drops." }
      ],
      clusters: [
        { category: "Product Quality", issue: "Top handle stitch failures", frequency: 14 },
        { category: "User Experience", issue: "Strap adjuster slipping", frequency: 10 },
        { category: "Delivery", issue: "Shipped in tight poly bag causing creases", frequency: 5 },
        { category: "Customer Service", issue: "Sent replacement strap quickly", frequency: 15 },
        { category: "Pricing", issue: "Affordable daily storage option", frequency: 22 }
      ],
      fixes: [
        "Incorporate double lockstitch construction on all stress joints.",
        "Upgrade bottom panels to 8mm high-density EVA shock protection."
      ],
      improvements: [
        "Add a luggage trolley pass-through sleeve on the back panel.",
        "Upgrade plastic buckle hardware to matte lightweight alloy."
      ],
      roadmap: [
        "Design a modular attachment system for external travel pouches.",
        "Launch an eco-friendly series constructed from recycled ocean nylon fibers."
      ],
      gaps: [
        "Our organization system outclasses competitors, but we lack integrated security zippers.",
        "Competitors bundle matching cable wraps which our model currently excludes."
      ],
      reviews: [
        `Tons of storage space and fits my laptop perfectly. The ${productName} looks very clean!`,
        `Excellent organization dividers, but the shoulder seam stitching is fraying after a month.`,
        `The pockets are great, but the bottom is thin and needs thicker shock foam.`,
        `Material blocks rain very well. Kept my tech dry during a heavy storm.`,
        `Decent bag, but the plastic buckles feel a bit fragile when fully loaded.`
      ]
    };
  }

  // Standard category dynamic mapping
  let baseTemplate = MOCK_TEMPLATES_MAPPING(cat, productName);
  return baseTemplate;
}

function MOCK_TEMPLATES_MAPPING(cat: string, productName: string): AspectSet {
  // Let's implement standard categories
  if (cat === "wearable") {
    return {
      summary: `Reviewers of ${productName} are generally pleased with its fitness tracking capabilities and modern design. The AMOLED screen is bright, and notifications are reliable. However, the heart-rate sensor occasionally lags during high-intensity training, and the proprietary charger is easy to misplace.`,
      pros: [
      ...
      comparison: `Compared to similar fitness trackers, the ${productName} excels in screen brightness but the companion app is slightly less refined than leading competitors.`,
      highlights: [
      ...
      reviews: [
        `The smartwatch tracker of this ${productName} works perfectly. Setup was straightforward.`,
        `Beautiful display and design, but the battery drains twice as fast as my old tracker.`,
    ...
    return {
      summary: `Reviews for ${productName} emphasize its ease of use and consistent results. Coffee/meals are prepared quickly and maintain excellent temperature. However, the cleaning process is tedious due to non-removable base plates, and the steam valve is noisy.`,
      pros: [
      ...
      comparison: `The ${productName} heats faster than traditional heating pots, but lacks the granular temperature controls found in professional barista tools.`,
      highlights: [
      ...
      reviews: [
        `The brewer unit of this ${productName} works perfectly. Heating is extremely fast.`,
        `Beautiful stainless finish, but the steaming valve is much louder than my old cooker.`,
        `Excellent price point for a kitchen brewer. Stainless steel feels premium. Shipping took 4 days.`,
    ...
    return {
      summary: `Reviewers find the ${productName} highly comfortable and stylish for everyday wear. The fabric/material feels soft and holds up well in standard weather. However, the sizing runs noticeably small, and the stitching around the seams feels thin.`,
      pros: [
      ...
      comparison: `While more comfortable than traditional structured products, the ${productName} has lower durability for active trail use or heavy washing cycles.`,
      highlights: [
      ...
      reviews: [
        `The apparel items of this ${productName} feel extremely comfortable. Wearing comfort is top tier.`,
    ...
    return {
      summary: `Reviewers of ${productName} are highly impressed with its vibrant OLED display, snappy performance, and excellent camera system. The software user interface is smooth and fluid. However, battery life is average and charging speeds are sluggish without purchasing a separate fast-charging block.`,
      pros: [
      ...
      comparison: `Compared to other flagships, the ${productName} offers a superior display for the price, though it falls behind in telephoto camera zoom and fast charging capabilities.`,
      highlights: [
      ...
      reviews: [
        `The phone device of this ${productName} works beautifully. The OLED screen is stunningly bright and colorful.`,
    ...
    return {
      summary: `Reviewers of ${productName} are highly satisfied with its ergonomic grip, precise sensor tracking, and tactile click feedback. The wireless connectivity is responsive with minimal latency. However, some complain about scroll wheel rattle and side buttons placement.`,
      pros: [
      ...
      comparison: `Compared to standard office mice, the ${productName} offers superior ergonomic support and customizable DPI settings, though the scroll wheel feels less robust than premium gaming alternatives.`,
      highlights: [
      ...
      reviews: [
        `The mouse of this ${productName} works beautifully. The clicks feel tactile and responsive.`,
    ...
    return {
      summary: `Reviewers of ${productName} are highly satisfied with its typing comfort, tactile switch feedback, and sleek layout. The wireless connectivity is responsive with low latency. However, some complain about key stabilizer rattle on the spacebar and the lack of shine-through keycaps.`,
      pros: [
      ...
      comparison: `Compared to standard membrane keyboards, the ${productName} offers a far superior mechanical typing experience, though the key stabilizers feel less premium than custom-built alternatives.`,
      highlights: [
      ...
      reviews: [
        `The keyboard of this ${productName} works beautifully. The keystrokes feel tactile and responsive.`,
    ...
    return {
      summary: `Reviewers praise the ${productName} for its excellent vocal clarity, robust metal construction, and plug-and-play simplicity. It captures audio with minimal self-noise. However, users report that the desk stand picks up desk vibrations easily and the gain control dial is too loose.`,
      pros: [
      ...
      comparison: `Compared to built-in laptop microphones, the ${productName} provides professional broadcast-quality sound, though it requires a shock mount to match studio setups.`,
      highlights: [
      ...
      reviews: [
        `The microphone of this ${productName} works perfectly. The vocal clarity is crisp and clean.`,
    ...
    return {
      summary: `Reviewers of ${productName} are highly impressed with its vibrant colors, sharp resolution, and high refresh rate. The panel offers excellent viewing angles and thin bezels. However, many note noticeable backlight bleed in dark rooms and a slightly wobbly stand.`,
      pros: [
      ...
      comparison: `Compared to standard office displays, the ${productName} provides superior color space coverage (99% sRGB) and gaming response times, though the stand lacks height adjustments found in professional monitors.`,
      highlights: [
      ...
      reviews: [
        `The monitor screen of this ${productName} works beautifully. The colors are extremely vibrant and the contrast is excellent.`,
        `Love the high refresh rate, but the display panel has a bit of backlight bleed in the corners.`,
        `Excellent price point for a display monitor. The stand feels wobbly.`,
    ...
    return {
      summary: `Readers appreciate the insightful and life-changing concepts presented in ${productName}. The writing style is engaging and easy to follow. However, some find the later chapters repetitive and the paperback cover feels thin.`,
      pros: [
      ...
      comparison: `Compared to other bestsellers in personal growth, ${productName} offers a more structured approach, though some readers might prefer a more concise format.`,
      highlights: [
      ...
      reviews: [
        `The book of this ${productName} was an absolute joy to read. Very inspiring advice.`,
        `The concepts are interesting, but the middle chapters feel a bit repetitive and slow.`,
        `Excellent price point for a bestseller book. The formatting is clear and high-quality paper. Shipping took 4 days.`,
    ...
    return {
      summary: `Reviewers of ${productName} are extremely pleased with its organizational capacity and sleek aesthetic. It declutters work spaces effectively and the metal/wood parts feel sturdy. However, the assembly instructions can be hard to follow, and the drawer tracks feel slightly loose over time.`,
      pros: [
      ...
      comparison: `Compared to cheap plastic trays, the ${productName} offers superior structural rigidity, though it occupies slightly more desk area than standard models.`,
      highlights: [
      ...
      reviews: [
        `The desk organizer of this ${productName} helps declutter my desk perfectly. Setup was clean.`,
  ...
  // Fallback to fully customized dynamic generic templates based on the product name words
  const cleanName = productName.replace(/[^a-zA-Z0-9\s]/g, "").trim();
  const words = cleanName.split(/\s+/).filter(w => w.length > 2 && !/^(and|the|for|with|buy|from|online|price|under|off|sale|ltd|inc|pvt|co)$/i.test(w));
  const noun = words[words.length - 1] || "Product";
  const capitalNoun = noun.charAt(0).toUpperCase() + noun.slice(1);

  return {
    summary: `Reviewers generally rate the ${productName} highly for its usability and solid build quality. The design is modern and fits well into daily use. However, some users find it difficult to clean and note that the instruction sheet could be clearer.`,
    pros: [
      { term: `Excellent ${capitalNoun} Build`, count: 20, percentage: 80.0 },
      { term: `Intuitive ${capitalNoun} Usability`, count: 16, percentage: 70.0 },
      { term: `Premium Material Feel`, count: 12, percentage: 55.0 },
      { term: "Value for the Price", count: 9, percentage: 40.0 }
    ],
    cons: [
      { term: `Difficult to Clean ${capitalNoun}`, count: 15, percentage: 48.0 },
      { term: `Brief ${capitalNoun} Instructions`, count: 10, percentage: 32.0 },
    ...
    comparison: `Compared to standard models on the market, the ${productName} offers a more modern aesthetics, though some details could be refined for long-term durability.`,
    highlights: [
      { category: "Usability", quote: `Very practical for daily use. The ${noun} holds up well under normal conditions.`, sentiment: "Positive" },
    ...
    complaints: [
      { issue: `Vague instructions & ${noun} setup details`, impact_score: 6, volume: 12, severity: "Medium", root_cause: "The packaging brochure lacks clear diagrams for assembly and care." },
      { issue: `Difficult to deep clean inner seams`, impact_score: 5, volume: 10, severity: "Low", root_cause: "Tight angles and crevices trap moisture and dust easily." },
    ...
    timeline: [
      ...
      { stage_or_time: `Week 2 (${capitalNoun} break-in)`, issue: "Stiff latch or joints", diagnostic: "Moving parts require a break-in period to smooth out friction." },
      { stage_or_time: "Month 3", issue: "Bottom scuffs or pads loosen", diagnostic: "Repetitive washing or friction weakens the adhesive/rubber parts." }
    ],
    requests: [
    ...
    clusters: [
      ...
      { category: "User Experience", issue: `Hard to clean ${noun} slots`, frequency: 12 },
    ...
    fixes: [
      `Redesign ${noun} user manual with simplified visual steps and cleaning tips.`,
    ...
    reviews: [
      `The build quality of this ${productName} is solid. Fits perfectly into my daily routine!`,
      `Nice usability, but the instruction guide sheet is very brief and confusing.`,
      `Good value for the price, though the surface shows smudges and scratches easily.`,
      `Very practical and lightweight. Highly recommended utility product!`,
      `It works well for standard usage, but washing the tight corners is a bit difficult.`
    ]
  };
}
...
export function getMockReviewsForUrl(productName: string, url: string): string[] {
  ...
  const template = generateDynamicTemplate(display);
  return template.reviews.map(r => r.replace(/\$\{url\}/g, url).replace(/\{url\}/g, url));
}
...
export function getMockDataForProduct(productName: string, reviewsText: string = ""): any {
  const display = productName || "Product";
  return generateDynamicTemplate(display);
}
